...
- **The worker's budget is a share of the session's, not a number of its own.** `hive_worker_max_turns()` derives `AgentDefinition.maxTurns` from `MAX_TURNS` (a fifth of it, floored at 40) because the two are spent from the same pot — the cap bounds the whole query and the terminal `ResultMessage` aggregates lead plus every subagent. The old flat 200 against a 400-turn session meant two busy workers could end the session in `error_max_turns` with nothing committed, re-running the entire fanned-out task. `CLAUDETM_HIVE_WORKER_MAX_TURNS` still overrides; a project agent pins its own with `max_turns:` frontmatter, and one that doesn't now inherits the worker budget rather than running unbounded.
...
| `hive_worker_max_turns()` | `MAX_TURNS // 5`, min 40 | `CLAUDETM_HIVE_WORKER_MAX_TURNS` | Per-worker share of the session's turn budget — the two are the same pot |
