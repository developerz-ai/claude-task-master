...
- **The project's own `.claude/agents/` specialists are part of the team.** `list_project_agents()` (`core/subagents.py`) surfaces each project agent's name and description in the fan-out brief; a piece matching one is dispatched to that agent (`subagent_type: <name>`) instead of a generic `hive-worker`, under the same rules. Read per work session, so agents added mid-run are picked up.
