...
- **BREAKING: `--parallel-tasks` is replaced by `--parallel`, and parallelism is now on by default.** 0.1.83 shipped hive mode as *batching several tasks* of a PR group into one lead session, opt-in. That was the wrong unit. The task — not a list of tasks — is what gets parallelised: each task still gets its own work session, checks off exactly one task, and advances `current_task_index` by one, exactly as before. What is new is that the agent running that session is a **lead** which may split *its own single task* into pieces with disjoint write sets and dispatch `hive-worker` subagents for them, doing everything that overlaps itself. The batching, the `TASKS COMPLETE:` manifest, the multi-task check-off, the two-task floor and the `--pr-per-task` incompatibility are all **deleted** — none of them have anything to describe any more.
  ...
  The flag is `--parallel` / `--no-parallel` (`TaskOptions.parallel`, **default `True`**), with the same field through `config-update`, REST (`POST /task/init`, `PATCH /config`, `GET /status`) and MCP (`initialize_task`, `update_config`). `--parallel-tasks` and `parallel_tasks` are **gone, not aliased**: a replaced design is deleted here, so scripts and API clients passing the old name must be updated. Parallel is the default because the *decision* is free — the lead only spends anything if it finds real seams.
...
- **`auto_merge` no longer claims an approval gate that does not exist** (#146). Every user- and machine-facing description said PRs merge "when CI passes **and approved**" — CLI help, the REST models, the MCP tool specs and handlers, README, `docs/api-reference.md`, `docs/mcp-tools.md`, `docs/docker.md`. There is no approval concept anywhere in the code: no `reviewDecision`, no waiting-for-an-approver state, and `--admin` exists precisely to *override* a base-branch policy that requires one. All of them now describe the real gate — CI green, no unresolved review threads, no changes-requested review — and say plainly that an approving review is never required and never waited for. If you want a human checkpoint, that is `--no-auto-merge` or `--pause-on-pr`.
