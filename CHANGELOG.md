...
- **The session turn cap was written for a soloist and silently rationed the hive.** `MAX_TURNS` bounds the whole query and the terminal `ResultMessage` aggregates the lead plus every subagent — so at 400, a lead plus two busy 200-turn workers could exhaust the session, ending it in `error_max_turns` with nothing committed, which re-runs the entire fanned-out task. **`MAX_TURNS` now defaults to 2000**, sized for a lead plus a team. Workers keep their generous per-worker budget: squeezing each one to fit the old cap would have them stop mid-piece and hand the lead back work that was nearly done. (Project agents are no longer passed by claudetm at all — see the duplicate-registration entry below — so their frontmatter is the CLI's to interpret.)
...
- **Every project agent was registered twice.** `setting_sources` includes `"project"` (it must, or `CLAUDE.md` would not load either), which is what puts `.claude/agents/*.md` in the model's registry — and claudetm passed the same definitions again via `agents=`. The two copies are not deduplicated: a live session reported a project agent "listed twice, identically — a duplicate entry in the registry". Each specialist's full prompt was therefore in every fan-out query's system prompt twice — **~14k duplicated tokens on an 11-agent project, ~27k on a 16-agent one** — and with two identical entries nothing determined which copy a dispatch resolved to. claudetm now sends only its own built-ins, and stands down for any name the project defines, so a project `hive-worker.md` still wins.
...
- **BREAKING: `--parallel-tasks` is replaced by `--parallel`, and parallelism is now on by default.** 0.1.83 shipped hive mode as *batching several tasks* of a PR group into one lead session, opt-in. That was the wrong unit. The task — not a list of tasks — is what gets parallelised: each task still gets its own work session, checks off exactly one task, and advances `current_task_index` by one, exactly as before. What is new is that the agent running that session is a **lead** which may split *its own single task* into pieces with disjoint write sets and dispatch `hive-worker` subagents for them, doing everything that overlaps itself. The batching, the `TASKS COMPLETE:` manifest, the multi-task check-off, the two-task floor and the `--pr-per-task` incompatibility are all **deleted** — none of them have anything to describe any more.
  ...
  The flag is `--parallel` / `--no-parallel` (`TaskOptions.parallel`, **default `True`**), with the same field through `config-update`, REST (`POST /task/init`, `PATCH /config`, `GET /status`) and MCP (`initialize_task`, `update_config`). `--parallel-tasks` and `parallel_tasks` are **gone, not aliased**: a replaced design is deleted here, so scripts and API clients passing the old name must be updated. The REST request models say so out loud — `parallel_tasks` on `POST /task/init` or `PATCH /config` is a **422**, not a silently ignored key, because pydantic's default (drop it, apply `parallel`'s new default of `True`) would hand a client asking for *no* parallelism exactly the opposite. Parallel is the default because the *decision* is free — the lead only spends anything if it finds real seams.

  `CLAUDETM_HIVE_MAX_PARALLEL` (default 10) is unchanged in name and value but is documented for what it is: a **safety ceiling, not a target**. Nothing outside a session can tell how big a task is before it runs, so the team size is the lead's judgement — zero workers and one worker are both legitimate answers, and zero is the common one. Fan-out is not free: every worker pays a full cold start re-reading the repo (a sibling project was observed spawning four workers for four one-line edits). Workers share this one checkout — never a git worktree, never a per-agent directory — with each worker's exclusive file set as the only lock, and the lead alone runs git, so there is never a second writer to the index.

### Documentation

- **`auto_merge` no longer claims an approval gate that does not exist** (#146). Every user- and machine-facing description said PRs merge "when CI passes **and approved**" — CLI help, the REST models, the MCP tool specs and handlers, README, `docs/api-reference.md`, `docs/mcp-tools.md`, `docs/docker.md`. There is no *approval* gate anywhere in the code: no waiting-for-an-approver state, and `--admin` exists precisely to *override* a base-branch policy that requires one. Approval does not gate merging — but **requested changes do**: this release reads GitHub's `reviewDecision` and refuses to merge a PR sitting at `CHANGES_REQUESTED`, `--admin` included. All of them now describe the real gate — CI green, no unresolved review threads, no changes-requested review — and say plainly that an approving review is never required and never waited for. If you want a human checkpoint, that is `--no-auto-merge` or `--pause-on-pr`.
- **Docker docs no longer bind-mount a human's `~/.claude` into the agent container** (#100). Mounting `~/.claude:/home/claudetm/.claude:ro` hands the container an operator's personal OAuth bearer token: the agent then runs as, and bills, that person's account, the token cannot be scoped to least privilege, and it cannot be rotated without breaking that person's own Claude login. This caused a real cross-developer credential leak. `docs/docker.md` and the README now document the supported scoped-credential path instead — an **api-key profile** holding an API key and Anthropic-compatible base URL, injected as `ANTHROPIC_API_KEY` / `ANTHROPIC_BASE_URL`, with the OAuth credentials-file check skipped entirely so no `~/.claude` is read at all. The profile registry lives in a named volume and is selected per-run with `CLAUDETM_PROFILE`, so the agent's credential is revoked or rotated on its own. Where that scoped key is minted (an Anthropic key issued for the agent, or one from a central proxy) is deliberately out of scope here.
...
- **`claudetm merge-pr` / `fix-pr` had the same bug verbatim.** `run_fix_session` posted the comment replies — which resolve the threads on GitHub — straight after the agent session with nothing verified, so a session killed mid-turn reported a handled review for a fix still in the working tree. It now checks the same contract and refuses to resolve anything it cannot see on the PR. That path also merged with no dirty-tree guard, as did the verification fix-PR path; both now refuse and name the pending files.
- **A cut-off planning session no longer saves a truncated plan.** `run_planning_phase` reported no success flag at all, so a session that stopped partway through writing the task list was persisted as if complete — and every later stage treats `plan.md` as the whole job, so the missing tasks are never noticed. Newly reachable now that sessions carry a turn cap. It reports `success`/`subtype` like `run_work_session`, and the planner keeps any existing plan rather than overwriting it with a truncation. (`plan_updater` was audited and already correct: it validates the result parses to real tasks, preserves every completed task, and backs the plan up before overwriting.)
- **Pending-change listings keep their status column** — `strip()` was eating the leading space of the first line (`" M path"`), misaligning it against every other line.

### Changed
- Test hermeticity in both directions, after CI caught a case local runs could not: `_unpushed_commit_count` ran without a `cwd` and measured whatever repository the process sat in, so in CI it counted the PR's own commits as unpushed and every fix stage refused to advance. A new `tests/cli_commands/conftest.py` neutralizes the CLI guards' ambient cwd reads, and the `pr_created` block test no longer depends on which branch the developer happens to be on — read from the real tree, it asserted "blocked" only while sitting on the base branch.
